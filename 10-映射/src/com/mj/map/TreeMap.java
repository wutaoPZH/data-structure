...
/**
 * 我们从头到尾实现一个map集合,我们重构一颗红黑树，这时候相当与TreeMap就是一颗红黑树。
 * @author 涛宝宝
 *
 */
public class TreeMap<K,V> implements Map<K, V> {
	...
	public boolean containsValue(V value) {
		//我们使用层序遍历来写一遍；
//		cheackValue(value);value的值是可以为null的；
		Queue<Node<K, V>> queue = new LinkedList<>();
		queue.offer(root);
		while (!queue.isEmpty()) {
			Node<K,V> poll = queue.poll();
			if (valueEquals(value,poll.value)) {
				return true;
			}
			if (poll.left != null) {
	...
	private boolean valueEquals(V value, V value2) {
